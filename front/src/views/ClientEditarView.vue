...
onMounted(() => {
  ...
  const requestClient = getClientFor(route.params.id);

  requestClient.then(function(response){
...
async function edit(){
    try {
      const data = await ClientDataService.update(route.params.id,{
        "name":name.value,
...
async function getClientFor(id) {
  return await ClientDataService.get(id);
}
</script>
